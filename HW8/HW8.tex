...
\begin{enumerate}
    ...
    \newpage
    \item Since $u\in \mathbb{R}^p$, $y = a_1e_1 + a_2e_2 + \cdots + a_pe_p$.
    Then, \begin{align*}
        Dg(x)(u_ie_i) &= u_i\cdot \lim_{h\to 0}\dfrac{g(x+he_i) - g(x)}{h}\\
        &= u_i\cdot \lim_{h\to 0} \dfrac{B(x, he_i) + B(he_i, x) + g(he_i)}{h}\\
        &= u_i(B(x, e_i) + B(e_i, x))\\
        &= B(u_i, x) + B(x, u_i)
    \end{align*}
    Thus, $Dg(x)(u) = B(x, u) + B(u, x)$ and using the same way can get $Dg(u)(x) = B(x, u) + B(u, x)$.
