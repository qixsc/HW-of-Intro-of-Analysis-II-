...
\begin{enumerate}
    ...
    That is, we can find union of open intervals $\cup_N I_n$ in $E$ with $|I_1| \geq |I_2|\geq \cdots$, and we let $L = \displaystyle\sum_{n=1}^N|I_n|$.
    
    Suppose $M_0 = \sup f(x)$ and $\alpha_0 = \displaystyle\int_{0}^{1} f(x)\ dx$,
    We want to show that there exists finite $N_0$ s.t. $\displaystyle\sum_{i=1}^{N_0} |I_n| \geq \dfrac{\alpha_0}{4M_0}$.
    
    Since there exists a partition $P$ s.t. $\displaystyle\int_{0}^{1} f(x)\ dx - L(f, P) \leq \dfrac{\alpha_0}{4}$,
    Then, 
