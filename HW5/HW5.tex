...
\begin{enumerate}
    ...
    For any $x_0\in S$, we let $x_k = f^k(x)$ and we can find a $N\in\mathbb{N}$ s.t. $a_n \leq \alpha < \dfrac{1}{d(x_1, x_0)}$ for all $n > N$.
    Then, for any $m > n > N$, $d(x_m, x_n) \leq \displaystyle\sum_{k=0}^{m-n-1} d(x_{n+k+1}, x_{n+k}) \leq \displaystyle\sum_{k=0}^{m-n-1} \alpha\cdot d(x_1, x_0)$
